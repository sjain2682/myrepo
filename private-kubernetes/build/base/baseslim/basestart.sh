...
set_user() {
    ...
    if getent passwd $MAPR_UID > /dev/null 2>&1 ; then
        ...
        useradd -G $MAPR_GROUP "mapr"
    else
        # create the new group with the given group id
        msg "Creating User: mapr UID: $MAPR_UID GID: $MAPR_GID"
        RESULTS=$(useradd -m -u $MAPR_UID -g $MAPR_GID -G $(stat -c '%G' /etc/shadow) mapr)
    fi
    #$MAPR_HOME/server/config-mapr-user.sh -u $MAPR_USER
    echo "root:$MAPR_ROOT_PASSWORD" | chpasswd
    echo "mapr:$MAPR_PASSWORD" | chpasswd
    success $YES
}
