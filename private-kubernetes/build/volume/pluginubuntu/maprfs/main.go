...
func cleanup() {
  Plugin.Println("INFO  Cleaning up old mounts")
	_, err := os.Stat(client_support_path); if err == nil {
		files, err := ioutil.ReadDir(client_support_path); if err != nil {
		...
		for _, file := range files {
				...
				kpathinfo, err := ioutil.ReadFile(kpathfile); if err != nil {
...
func linkFiles() {
  ...
  err := os.Symlink(from, to) ; if err != nil {
...
func startFuse(kpath string, options map[string]string) string {
  ...
  */
  ...
  // create FUSE directory
	...
	err := os.MkdirAll(fmount, 0700); if err != nil {
	...
	err = os.MkdirAll(spath, 0700); if err != nil {
  ...
  if sectype == "secure" {
		...
		info, err := ioutil.ReadFile(infofile); if err != nil {
		...
		err = ioutil.WriteFile(ticketfile, sDec, 0600); if err != nil {
...
func stopFuse(fmount string) error {
  ...
  // unmount FUSE
	_, err := os.Stat(fmount); if err == nil {
...
func doMount(args []string) {
  ...
  Plugin.Printf("INFO  Creating bind mount directory %s...", mpath)
	err := os.MkdirAll(mpath, 0700); if err != nil {
...
func unmount(args []string, cleanup bool) {
  ...
  Plugin.Println("INFO  Unmount bind mount...")
	_, err := os.Stat(kpath); if err == nil {
	...
	// do fuse unmount
	err = stopFuse(fmount); if err != nil {
		error = true
	}
	// read fusepid
	fpidfile := spath + "/fusepid"
	fpid, err := ioutil.ReadFile(fpidfile); if err != nil {
			...
			// kill fuse
			syscall.Kill(ipid ,syscall.SIGKILL); if err != nil {
	...
	// Remove support dir
  _, err = os.Stat(spath); if err == nil {
...
func doInit() {
  Plugin.Println("INFO  === Starting init of MapRfs Plugin ===")
	_, err := os.Stat(save_file); if err != nil {
			cleanup()
  		linkFiles()
  		...
  		hostFile := k8s_dir + "/hostname"
			err = ioutil.WriteFile(hostFile, out, 0600); if err != nil {
  		...
  		patternFile := "/proc/sys/kernel/core_pattern"
			err = ioutil.WriteFile(patternFile, []byte("/opt/cores/%e.core.%p.%h"), 0600); if err != nil {
...
func main() {
  args := os.Args
  op := os.Args[1]
	f, err := os.OpenFile(plugin_log, os.O_CREATE | os.O_APPEND|os.O_WRONLY, 0600); if err != nil {
